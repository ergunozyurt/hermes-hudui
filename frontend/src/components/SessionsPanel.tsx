...
export default function SessionsPanel({ selectedProfile }: { selectedProfile: string }) {
  ...
  return (
    <>
      <Panel title={`Session Activity · ${profileLabel(data?.profile || selectedProfile)}`} className="col-span-2">
      ...
      <Panel title={`Recent Sessions · ${profileLabel(data?.profile || selectedProfile)}`}>
