...
export default function CronPanel({ selectedProfile }: { selectedProfile: string }) {
  ...
  if (!Array.isArray(jobs) || jobs.length === 0) {
    return <Panel title={`Cron Jobs · ${profileLabel(data?.profile || selectedProfile)}`} className="col-span-full"><div className="text-[13px]" style={{ color: 'var(--hud-text-dim)' }}>No cron jobs configured</div></Panel>
  }

  return (
    <Panel title={`Cron Jobs · ${profileLabel(data?.profile || selectedProfile)}`} className="col-span-full">
