...
import { profileLabel, withProfile } from '../lib/profile'
...
export default function TokenCostsPanel({ selectedProfile }: { selectedProfile: string }) {
  ...
  return (
    <>
      {/* Today */}
        <Panel title={`Today · ${profileLabel(data?.profile || selectedProfile)} — $${today.estimated_cost_usd.toFixed(2)}`}>
      ...
      {/* All time */}
        <Panel title={`All Time · ${profileLabel(data?.profile || selectedProfile)} — $${allTime.estimated_cost_usd.toFixed(2)}`}>
        <div className="grid grid-cols-2 gap-3 mb-3">
      ...
      {/* Per-model breakdown */}
      {byModel.length > 0 && (
        <Panel title={`By Model · ${profileLabel(data?.profile || selectedProfile)} — ${byModel.length} models`} className="col-span-full">
